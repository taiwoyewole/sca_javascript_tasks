...
runners.forEach(newList => {
  fullNames = newList.first_name + ' ' + newList.last_name;
...
let runnersLargeSizeShirt = runners.filter(bigRunners => bigRunners.shirt_size === 'L');
console.log(runnersLargeSizeShirt);

// ==== Challenge 4: Use .reduce() ====
// The donations need to be tallied up and reported for tax purposes. Add up all the donations and save the total into a ticketPriceTotal variable.
let ticketPrice =runners.map(runner => runner.donation);
console.log(ticketPriceTotal);

let ticketPriceTotal = ticketPrice.reduce((accumulator, currentValue) => accumulator + currentValue);
console.log(ticketPriceTotal);
